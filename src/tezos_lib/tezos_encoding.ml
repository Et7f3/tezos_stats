...
*)
  ...
  let script_expr_encoding =
    ...
    mu "tezos_script" ~title:"Script expression" (fun expr_encoding ->
        union
          [ case int_encoding
...
module Base = struct
  ...
  module Block_header = struct
    ...
    let block_shell_encoding =
      conv
        (fun { shell_level; shell_proto; shell_predecessor; shell_timestamp;
               shell_validation_pass; shell_operations_hash; shell_fitness;
               shell_context } ->
          let shell_timestamp = Date.to_string shell_timestamp in
          let shell_fitness = String.split_on_char ' ' shell_fitness in
        ...
        (fun (shell_level, shell_proto, shell_predecessor, shell_timestamp,
              shell_validation_pass, shell_operations_hash, shell_fitness,
              shell_context) ->
          let shell_timestamp = Date.from_string shell_timestamp in
          let shell_fitness = String.concat " " shell_fitness in
          { shell_level; shell_proto; shell_predecessor; shell_timestamp;
            shell_validation_pass; shell_operations_hash; shell_fitness;
            shell_context })
        (obj8
      ...
      (conv
         (fun { node_or_src ; node_or_fee = _; node_or_counter = _ ;
           ...
           (((), node_or_src, ~-1, Some node_or_manager, Some node_or_manager, node_or_balance,
             node_or_spendable, node_or_delegatable,
  ...
  let block_shell_gen_encoding =
    conv
      ...
      (fun (shell_header, shell_header_protocol_data) ->
         { shell_header ; shell_header_protocol_data })
      (merge_objs
         Block_header.block_shell_encoding
